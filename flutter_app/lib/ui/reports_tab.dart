...
class _ReportsTabState extends State<ReportsTab> {
  ...
  Widget build(BuildContext context) {
    ...
    // Filtro básico: não usar ignorados em status ou totais (conforme toggle se existir)
    var validTransactions = provider.transactions.where((t) => !t.ignoreInStats).toList();

    // Filtra transações efetivadas apenas, se for regime de caixa
    if (!_useCompetencia) {
      validTransactions = validTransactions.where((t) => t.isPaid).toList();
    }
    
    double expenses = validTransactions.where((t) => t.type == TransactionType.expense || t.type == TransactionType.creditCardExpense).fold(0, (sum, t) => sum + t.amount);
    double income = validTransactions.where((t) => t.type == TransactionType.income).fold(0, (sum, t) => sum + t.amount);
        ...
        children: [
          Row(
            ...
            children: [
              ...
              IconButton(onSettingsPressed: () {}, icon: const Icon(Icons.settings_outlined))
            ],
          ),
          ...
          Container(
            ...
            child: total == 0 
              ? const SizedBox(
                  height: 200,
                  child: Center(child: Text('Nenhuma transação encontrada.', style: TextStyle(color: Colors.black45, fontWeight: FontWeight.w500)))
                )
              : Column(
                  children: [
                    SizedBox(
